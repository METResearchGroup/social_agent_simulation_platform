...
def upgrade() -> None:
    ...
    with op.batch_alter_table("bluesky_feed_posts", schema=None) as batch_op:
        batch_op.create_index(
            "idx_bluesky_feed_posts_author_handle", ["author_handle"], unique=False
        )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "((status = 'completed' AND completed_at IS NOT NULL AND completed_at >= started_at) OR (status != 'completed' AND completed_at IS NULL))",
    ...
    with op.batch_alter_table("runs", schema=None) as batch_op:
        batch_op.create_index(
            "idx_runs_created_at",
            [sa.column("created_at").desc()],  # type: ignore[reportArgumentType]
            unique=False,
        )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.Text(), nullable=False),
        sa.CheckConstraint(
            "turn_number >= 0", name="ck_turn_metadata_turn_number_gte_0"
        ),
