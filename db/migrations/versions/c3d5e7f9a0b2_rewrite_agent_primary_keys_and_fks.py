...
foreign_keys=ON``, ``PRAGMA foreign_key_check`` runs for each table this
revision updates (not the whole DB, so unrelated legacy rows cannot fail the
upgrade) before canonical-format validation.
...
# Tables whose rows are updated by this migration — scope PRAGMA foreign_key_check to these only.
_AGENT_MIGRATION_FK_CHECK_TABLES: tuple[str, ...] = tuple(
    dict.fromkeys(
        [t for t, _ in _AGENT_FK_COLUMNS]
        + [t for t, _ in _RUN_AGENT_FK_COLUMNS]
        + ["run_agents", "agent"]
    )
)
...
def _assert_foreign_key_integrity(connection) -> None:
    """Run SQLite referential integrity check on tables touched by this migration."""
    rows: list[tuple[object, ...]] = []
    for table in _AGENT_MIGRATION_FK_CHECK_TABLES:
        # Table names are migration-local constants only (not user input).
        chunk = connection.execute(
            text(f'PRAGMA foreign_key_check("{table}")')  # nosec B608
        ).fetchall()
        rows.extend(chunk)
    if not rows:
        return
