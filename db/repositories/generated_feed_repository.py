...
class SQLiteGeneratedFeedRepository(GeneratedFeedRepository):
    ...
    def write_generated_feed(
        ...
        Raises:
            ValueError: If required feed fields are invalid (validated by Pydantic model)
    ...
    def get_generated_feed(
        ...
        Note:
            ...
            parameters (not a GeneratedFeed model), we validate agent_id and run_id here.
        """
        with self._transaction_provider.run_transaction() as c:
            return self._db_adapter.read_generated_feed(
                agent_id, run_id, turn_number, conn=c
            )
    ...
    def get_post_ids_for_run(self, agent_id: str, run_id: str) -> set[str]:
        ...
        """
        with self._transaction_provider.run_transaction() as c:
            return self._db_adapter.read_post_ids_for_run(agent_id, run_id, conn=c)
