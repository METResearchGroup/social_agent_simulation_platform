"""Central registry for feed generation algorithms."""

from collections.abc import Callable

from feeds.algorithms.implementations.chronological import (
    METADATA as chronological_metadata,
)
from feeds.algorithms.implementations.chronological import (
    generate as generate_chronological,
)
from feeds.algorithms.interfaces import FeedAlgorithmMetadata

_ALGORITHM_LOOKUP: dict[str, tuple[Callable[..., dict], FeedAlgorithmMetadata]] = {
    "chronological": (generate_chronological, chronological_metadata),
}

FEED_ALGORITHMS: tuple[str, ...] = tuple(_ALGORITHM_LOOKUP.keys())


def get_registered_algorithms() -> list[tuple[str, FeedAlgorithmMetadata]]:
    """Return list of (algorithm_id, metadata) for all registered algorithms."""
    return [(alg_id, meta) for alg_id, (_, meta) in _ALGORITHM_LOOKUP.items()]


def get_feed_generator(algorithm: str) -> Callable[..., dict]:
    """Return the feed generator callable for the given algorithm.

    Raises:
        ValueError: When algorithm is not registered.
    """
    entry = _ALGORITHM_LOOKUP.get(algorithm)
    if entry is None:
        raise ValueError(f"feed_algorithm must be one of: {list(FEED_ALGORITHMS)}")
    return entry[0]
