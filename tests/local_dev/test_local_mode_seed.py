...
class TestLocalModeSeed:
    ...
    def test_seed_local_db_if_needed__idempotent(self, temp_db, caplog) -> None:
        ...
        try:
            ...
            agent_post_comments_count = conn.execute(
            ...
            assert agent_post_comments_count > 0
        ...
        try:
            ...
            agent_post_comments_count_before = conn.execute(
                "SELECT COUNT(*) FROM agent_post_comments"
            ).fetchone()[0]
        finally:
            conn.close()
        ...
        try:
            ...
            agent_post_comments_count_after = conn.execute(
                "SELECT COUNT(*) FROM agent_post_comments"
            ).fetchone()[0]
        finally:
            conn.close()
