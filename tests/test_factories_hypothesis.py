...
class TestFactoriesHypothesis:
    @given(post=bluesky_post_strategy())
    @settings(max_examples=50, deadline=None)
    def test_bluesky_post_strategy_produces_valid_post(self, post):
        assert post.uri
        assert post.post_id == f"bluesky:{post.uri}"
        assert post.source.value == "bluesky"
    ...
    def test_run_config_strategy_produces_valid_config(self, config):
