...
> **TODO:** This file will be deprecated in favor of `simulation/cli/main.py` in a future PR.
...
> **TODO:** We can get arbitrarily complex with how we do this later on, but as a first pass it's easy enough to just load all the posts.
...
> **TODO:** Right now we load all posts per agent, but we can optimize and personalize later to save on queries.
...
> **NOTE:** Choosing to do this here instead of `__init__` so that we can use the classmethods while assuming that the providers are already imported.
...
> **TODO:** For now, we support only Bluesky posts being added to feeds. We'll revisit how to add AI-generated posts to feeds later on.
