...
## Happy Flow
1. Run `uv run python scripts/lint_pytest_class_convention.py` (see `scripts/lint_pytest_class_convention.py`) to detect module-level test functions under `tests/`.
...
4. Run `uv run pre-commit run --all-files` to ensure the new hook (`python_test_class_convention` in `.pre-commit-config.yaml`) passes.
5. Confirm GitHub Actions runs the dedicated job (`pytest_test_class_convention` in `.github/workflows/ci.yml`) to prevent regressions.

## Data Flow
1. Developer adds/edits tests under `tests/`.
2. Pre-commit and CI invoke `scripts/lint_pytest_class_convention.py` which AST-parses `tests/**/test_*.py`.
...
## Manual Verification
- [ ] `uv sync --extra test`
- [ ] `uv run python scripts/lint_pytest_class_convention.py`
  - Expected (after migration): exits `0` and prints `OK: no module-level test_* functions found`.
- [ ] `uv run pytest tests/lint/test_lint_pytest_class_convention.py`
- [ ] `uv run pytest`
