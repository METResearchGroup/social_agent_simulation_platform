...
Frontend — State lifecycle invariants: Any component with (context key) + async action must implement: (1) reset-on-key-change, (2) clear-error-on-attempt-start, (3) request-id/guard for async completion, (4) keyed state if multiple entities can be acted on. For any UI state that is scoped to a context key (e.g. selectedAgentHandle, runId, threadId, route param), you must define and enforce these invariants:

- Reset on scope change: When the context key changes, reset all state that would be misleading in the new scope (errors, pending flags, derived selections, optimistic UI). Example: You tried deleting Agent A, got deleteError="Permission denied". Then you click Agent B. Without this rule, Agent B’s header might still show “Permission denied” even though you haven’t tried deleting B.
- Reset on new attempt: At the start of any async action, clear prior outcome state from the same action (error=null, success=false, etc.) before setting loading flags. Example: You press “Delete” again after a failure. If you don’t clear deleteError before setting isDeleting, the UI might keep showing the old error during the new attempt (or briefly flash it), which confuses users.
- Guard against stale async completion: Async results must only update state if they still match the latest context key / request id (prevent out-of-order writes). Example: You select Agent A, fetch details (Request 1). Quickly select Agent B, fetch details (Request 2). If Request 1 resolves after Request 2, it could overwrite the UI with Agent A’s details while Agent B is selected.
- Avoid cross-scope leakage: Don’t store per-entity state in a single top-level variable unless it’s keyed (e.g. errorById, isDeletingById) or reset on key change. If the UI can manage actions for multiple entities at once, a single isDeleting / deleteError boolean/string is often wrong because it’s not clear which entity it refers to.
